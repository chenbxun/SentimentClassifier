...
class SentimentClassifier(nn.Module):
    ...
    def init_hidden(self, batch_size, device):
        return (torch.randn(2*self.num_layers, batch_size, self.hidden_dim // 2, device=device),
                torch.randn(2*self.num_layers, batch_size, self.hidden_dim // 2, device=device))
